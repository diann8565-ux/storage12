...
import { HardDrive, FileIcon, Database, Upload, TrendingUp, Clock, Cloud } from "lucide-react";
...
export default function Dashboard() {
  const [stats, setStats] = useState<Stats>({ totalFiles: 0, totalSize: 0, activeAccounts: 0, apiUploads: 0 });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const activeAccounts = accounts.filter((a: { is_active?: boolean }) => Boolean(a.is_active));

        // Fetch logs for API stats
        const { data: logs } = await api.activityLogs.list();
        const apiUploads = logs.filter((l: LogItem) => l.action_type === 'API_UPLOAD').length;
  ...
  const statCards = [
    ...
    {
      title: "API Uploads (Recent)",
      value: stats.apiUploads.toString(),
      icon: Cloud,
