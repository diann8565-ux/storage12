...
export default function StorageAccounts() {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Periksa kembali kredensial Anda";
      toast.error(`Koneksi Gagal: ${message}`, { id: toastId });
    } finally {
      setIsTesting(false);
    }
  };
  ...
  const downloadTemplate = (provider: "imagekit" | "cloudinary", format: "json" | "csv") => {
    let content = "";
    const filename = `template_${provider}_account.${format}`;
